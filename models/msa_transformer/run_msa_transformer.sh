#!/bin/bash
# ============================================================
# run_msa_transformer.sh — Submits one SLURM GPU job per segment (+ ALL combined)
#                          for MSA Transformer 100M + MLP, then plots results.
#
# Usage:  bash run_msa_transformer.sh
# ============================================================
...
# ── Read segments + thresholds from best_per_segment.csv ─────────────────────
    ...
    JID=$(sbatch --parsable \
        ...
        --wrap="
            ...
            pip3 install --target=/workspace/.pip_packages \
                ...
                2>/dev/null
            python3 /workspace/msa_transformer_baseline.py \
                --segment      ${SEG} \
                --threshold    ${THR} \
                --split_dir    /workspace/splits_segmentwise_species \
                --output_dir   /workspace/msa_results/${SEG} \
...
JID_ALL=$(sbatch --parsable \
    ...
    --wrap="
        ...
        python3 /workspace/msa_transformer_baseline.py \
            ...
            --threshold all \
            --split_dir /workspace/splits_segmentwise_species \
            --output_dir /workspace/msa_results/ALL_combined \
            --msa_file  /workspace/data/pgen.1008079.s010.fas \
            --n_seqs    63 \
            --batch_size 2
    ")
